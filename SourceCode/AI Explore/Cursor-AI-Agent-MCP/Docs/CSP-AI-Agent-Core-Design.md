...
│   └── api/
│       └── csp-client.ts           # CSP REST API 客户端
│
├── CSP REST API (独立服务)
│   ├── resources/                  # 资源管理
│   ├── subscriptions/              # 订阅管理
...
├─ 下载操作 → REST API 从 Git 仓库读取文件 → 返回文件
└─ 上传操作 → REST API 执行 git add/commit/push → 返回结果
  ↓
MCP Tool 接收 REST API 响应
  ↓
【如果是下载资源】MCP Server 通过 git pull 获取最新文件到本地
  ↓
通过 SSE 返回结果给 Cursor IDE
  ↓
Cursor IDE 将资源文件写入用户本地目录
  (~/.cursor/rules/, ~/.cursor/skills/ 等)
```

**关键说明**:

1. **MCP Tools 执行位置**: 在 CSP Server 上执行，不是在用户本地
2. **API 调用**: MCP Server 调用同一台服务器上的 CSP REST API
3. **Git 操作**: MCP Server 通过 Git 命令管理本地仓库
4. **数据库**: CSP REST API 自己管理数据库，与 MCP Server 无关
5. **文件传输**: 最终资源文件通过 REST API 下载到用户本地
