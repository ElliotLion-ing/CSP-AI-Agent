...
- ✅ 精准遥测(track_usage) - Command/Skill 调用次数按用户、资源、Jira Issue 聚合上报
...
#### Command / Skill — MCP Prompt 模式 (v1.5)
...
3. **API 调用**: MCP Server 调用同一台服务器上的 CSP REST API
4. **Git 操作**: MCP Server 通过 Git 命令管理本地仓库
5. **数据库**: CSP REST API 自己管理数据库，与 MCP Server 无关
...
| 资源类型 | 统计内容 | 更新时机 |
|---------|---------|---------|
| Command / Skill | 精准调用次数（含 jira_id 分维度聚合） | GetPrompt handler 触发 |
