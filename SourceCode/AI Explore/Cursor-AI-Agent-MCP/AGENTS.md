...
| 11 | **用户审查机制** | 重要修复必须经过用户三问审查：范围/API/兼容性 |
...
**问题 2: API 使用验证**
- 是否使用 `Docs/Design/CSP-AI-Agent-API-Mapping.md` 定义的 API？
- 是否自己捏造了不存在的 API？

**问题 3: 跨平台兼容性**
- 是否考虑 Windows 和 macOS 差异？
- 是否处理所有相关路径 (global / workspace / .csp-ai-agent)？

### 审查标准

- ✅ **三问全部通过** → 修复有效，可以继续
- ❌ **任一问题失败** → 立即修复问题，重新测试，再次审查

### 示例: BUG-2026-04-07-001 修复流程

**v1 审查结果:**
```
1. 修复范围: ✅ 通过 (限定在 check 模式)
2. API 使用: ✅ 通过 (使用正确的 downloadResource API)
3. 兼容性: ❌ 失败 (只检查 global 路径,忽略 workspace 和 .csp-ai-agent)
```
...
- v2.4.0: 用户审查机制（规则 #11）——强制三问审查（范围/API/兼容性）
